...
impl Path {
    ...
    pub fn find_accessible_neighbors(map: &Map, (ax, ay, az): (i32, i32, i32), distance: usize, accessible_blocks: &mut AccessibleBlock, accesses: &mut Accesses) {
        ...
        let check_uphill_neighbor = |x, z| -> bool {
            ...
            if map.get_block(x, ay + 1, z) == Block::Air && map.get_block(x, ay + 2, z) == Block::Air && map.get_block(x, ay, z) != Block::Air && map.get_block(ax, ay+2, az) == Block::Air {
    ...
    pub fn find_path(
        ...
        trace!("There is at least {} accessible blocks (found in {} iterations)", accessible_blocks.len(), counter);
